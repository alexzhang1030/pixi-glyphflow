...
- WebGL dirty uploads of the `rgba32float` transform table `texImage2D` the existing texture.
  A packed full-width `texSubImage2D` (1024×1 at `x = 0`, `UNPACK_*` reset, `glError` 0) still
  blanked the first view after the initial allocation. Pixi `source.update()` on a same-size
  buffer texture is also `texSubImage2D`. WebGPU keeps the tight rectangles.

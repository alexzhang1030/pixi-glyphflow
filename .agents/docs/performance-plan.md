...
- Budget atlas uploads per frame and resume across frames. A 20,000-glyph first miss must not hitch a single commit. First-seen layout and raster already stop after `prepareBudgetMs` (default 8ms) or one `prepareWave` when the budget is 0. `TextLayer` continues admission on the next animation frame.

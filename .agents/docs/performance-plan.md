...
WebGL 2 keeps the Wave 1 CPU grid. The compute-cull slice is in the tree: WebGPU uploads label AABBs, prefix-sum + scatter compacts visible 24-byte instances in z then insertion order, and a Pixi encoder hook issues `drawIndexedIndirect`. `stats.cullPath` names `compute-cull` vs `cpu-grid`. Camera-only commits skip the CPU grid walk for render residency.

...
- [Paid traps](gotchas.md) — compute-cull working-set residency, compact-mesh veto after late glyph allocation, WGSL reserved `from`, PixiJS 128 MiB storage-binding default, CI Chrome/`float16x4` instance attributes, packed atlas keys vs `glyphText`, and the deferred atlas-pressure frame gate.

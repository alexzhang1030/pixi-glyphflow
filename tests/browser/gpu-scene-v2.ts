...
declare global {
  interface Window {
    __gpuSceneV2Fixture: GpuSceneV2FixtureState;
  }
}

window.__gpuSceneV2Fixture = { done: false };

void run().catch((error: unknown) => {
  window.__gpuSceneV2Fixture = {
    done: true,
...
async function run(): Promise<void> {
  ...
  window.__gpuSceneV2Fixture = { done: true, result, pixels };
  app.destroy();
}

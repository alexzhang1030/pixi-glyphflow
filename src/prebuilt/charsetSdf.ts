 ...
 * encodes; later calls remap keys and logical `rasterScale`. A bake at one clamp-equivalent
 * logical size is enough: `RasterGlyphProvider` rematches the physical field. Pages stay out of
 * the core graph. Does not ship CJK bitmaps — the host paints, typically after `FontFace.load`.
 */

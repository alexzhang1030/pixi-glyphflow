...
const encoderHooks = new WeakMap<WebGPURenderer, Set<ComputeCullPass>>();

type EncoderDrawOptions = Parameters<WebGPURenderer["encoder"]["draw"]>[0];
...
export class ComputeCullPass {
  ...
  ownsGeometry(geometry: Geometry | undefined): boolean {
    return geometry !== undefined && this.#geometries.has(geometry);
  }
  ...
  ensureCapacity(labelCount: number, instanceBytes: number): void {
    ...
    if (labels > this.#labelCapacity) {
      ...
      this.#groupSums = createBuffer(
        device,
        Math.ceil(labels / CULL_WORKGROUP) * 4,
        GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
      );
  ...
  uploadInstances(instances: ArrayBuffer): void {
    const device = this.#device;
    if (device === undefined || this.#instancesIn === undefined || instances.byteLength === 0) {
      return;
    }
    device.queue.writeBuffer(this.#instancesIn, 0, instances, 0, instances.byteLength);
  }

  dispatch(viewport: CullViewport): void {
    const device = this.#device;
...
function installEncoderHook(renderer: WebGPURenderer, pass: ComputeCullPass): void {
  let passes = encoderHooks.get(renderer);
  if (passes === undefined) {
    passes = new Set();
    encoderHooks.set(renderer, passes);
    const encoder = renderer.encoder;
    const original = encoder.draw.bind(encoder);
...
function uninstallEncoderHook(renderer: WebGPURenderer, pass: ComputeCullPass): void {
  encoderHooks.get(renderer)?.delete(pass);
}
